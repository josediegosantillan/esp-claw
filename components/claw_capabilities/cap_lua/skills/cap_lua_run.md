...
2. Use `lua_run_script` for short work and `lua_run_script_async` for long-running work.
3. For built-in examples, run them from `builtin/*.lua` when checking baseline behavior. For iterative changes, copy them to a stable `temp/*.lua` path and keep stable `name` / `exclusive`.
4. Inspect state with `lua_list_async_jobs` first, then use `lua_get_async_job` when you need details.
5. When the user asks to stop a job, call the stop tool first and then answer based on the returned result.
